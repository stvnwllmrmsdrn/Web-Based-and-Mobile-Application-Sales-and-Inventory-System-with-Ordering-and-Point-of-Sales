    ...
    <div class="row">
        <div class="col-md-12">
                ...
                <ul class="nav nav-tabs" id="scroll" style="">
                        ...
                        <li class="active"><a href="customer-order-recieve" data-toggle="tab">To Receive<?php if ($to_receive == 0 ) { echo ''; }else { echo '<span class="count">'.$to_receive.'</span>';}?></a></li>
...
<?php
            ...
            $targetpage = BASE_URL.'customer-order-recieve.php';
            $limit = 10;
            $page = @$_GET['page'];
            if($page) 
                $start = ($page - 1) * $limit;
            ...
            $statement = $pdo->prepare("SELECT * FROM tbl_payment WHERE customer_email=? AND order_status!='Pending' AND shipping_status!='Pending' AND payment_status!='Pending' AND delivery_status='Pending' ORDER BY id DESC LIMIT $start, $limit");
            $statement->execute(array($_SESSION['customer']['cust_email']));
            ...
            <div class="d-flex">
              <?php
                if($row['shipping_status']=='Shipped Out') {
                        ?>
                        <a href="receive-change-status.php?id=<?php echo $row['id']; ?>&task=Completed" class="btn btn-primary btn-xs" style="width:100%;height:25px;margin-bottom:4px;">Order Received</a>
                        <?php
                }
              ?>
