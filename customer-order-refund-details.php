...
?>
     ...
     <table class="table table-borderless">
            ...
            <?php 
               if ( $os == 'Pending') {
                echo '<span class="badge rounded-pill bg-warning">Pending Request</span>';
                }elseif ( $os == 'Declined'){
                echo '<span class="badge rounded-pill bg-danger">Request Declined</span>';
                }else {
                echo '<span class="badge rounded-pill bg-success">Request Accepted</span>';
              ...
              <?php
                ...
                <td >
                  <div class="d-flex mb-2">
                    <div class="flex-shrink-0" style="align-self: center;">
                      <a href="product?id=<?php echo $row1['product_id']; ?>">
                        <img src="assets/uploads/refund_proof/<?php echo $row['photo']; ?>" alt="" width="80" class="img-fluid"></a>
          ...
          ?>
                    <div class="flex-lg-grow-12 ms-3">
                      <h6 class="small mb-0"><a href="product?id=<?php echo $row1['product_id']; ?>" class="text-reset fw-bold">Reason: <?php echo $row1['reason']; ?></a></h6>
                      <span class="small ">Notes: <?php echo $row['notes']; ?></span>
                      <span class="small">GCash Number: <?php echo $row['gcash_no']; ?></span>
                      <span class="small">Attempt: <?php echo $row['attempt']; ?></span>
                    </div>
                    <?php
          }
          ?>
            ...
            <tfoot>
              <tr class="fw-bold">
                <td colspan="2">
                  ...
                  if ( $os == 'Declined') {
                    echo 'Seller Notes: ';
                    echo '<textarea class="form-control" style="border-radius:15px;background-color: #d9edff;" cols="30" rows="4" disabled> '.$row['seller_notes'].'</textarea>';
                      if ( $at <= 2) { 
                        echo '<h6>You only have <b> 3 ATTEMPTS </b> to request for <b> REFUND</b>. Please check the details before submitting the request</h6>';?>
                        <h6>Please click the button below to resubmit proof of your request *</h6>
                        <a href="customer-order-refund-declined?id=<?php echo $row['id']; ?>" class="btn btn-primary btn-xs" style="width:100px;margin-bottom:4px;">Edit Details</a>
                      <?php 
                      }else {
