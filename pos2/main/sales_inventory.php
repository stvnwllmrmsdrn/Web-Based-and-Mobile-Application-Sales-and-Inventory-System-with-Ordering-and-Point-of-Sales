...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("SELECT s.transaction_id, s.name, so.invoice, so.date, so.p_name,so.p_current_price, so.qty, so.amount, s.profit, s.type FROM tbl_salesorder_pos AS so, tbl_sales_pos AS s WHERE so.transaction_id=s.transaction_id ORDER BY transaction_id DESC");
				$result->execute();
