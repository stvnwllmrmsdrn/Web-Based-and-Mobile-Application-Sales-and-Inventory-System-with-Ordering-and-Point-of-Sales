...
?>
            ...
            <div class="page-banner" style="background-image: url(assets/uploads/<?php echo $contact_banner; ?>);">
                <div class="inner">
                    <h1><?php echo LANG_VALUE_24; ?> | To Ship</h1>
                </div>
                </div>
        ...
        <div class="col-md-12">
                ...
                <div class="nav-tabs-custom">
                  <ul class="nav nav-tabs" id="scroll" style="">
                        ...
                        <li class="active"><a href="customer-order-ship" data-toggle="tab">To Ship<?php if ($to_ship == 0 ) { echo ''; }else { echo '<span class="count">'.$to_ship.'</span>';}?></a></li>
...
<?php
            ...
            $statement = $pdo->prepare("SELECT * FROM tbl_payment WHERE customer_email=? AND order_status!='Pending' AND order_status!='Declined' AND payment_status!='Pending' AND shipping_status='Pending' AND delivery_status='Pending' ORDER BY id");
            // $statement = $pdo->prepare("SELECT * FROM tbl_payment WHERE customer_email=? AND order_status!='Pending' AND payment_status!='Pending' AND shipping_status='Pending' AND delivery_status='Pending' ORDER BY id DESC");
            $statement->execute(array($_SESSION['customer']['cust_email']));
            ...
            $targetpage = BASE_URL.'customer-order-ship.php';
            $limit = 10;
            $page = @$_GET['page'];
            if($page)   
                $start = ($page - 1) * $limit;
            ...
            $statement = $pdo->prepare("SELECT * FROM tbl_payment WHERE customer_email=? AND order_status!='Pending' AND order_status!='Declined' AND payment_status!='Pending' AND shipping_status='Pending' AND delivery_status='Pending' ORDER BY id DESC LIMIT $start, $limit");
            $statement->execute(array($_SESSION['customer']['cust_email']));
