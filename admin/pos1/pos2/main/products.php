...
<?php 
			...
			?>
				<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM tbl_product where p_qty < reorder_level ORDER BY p_id DESC");
				$result->execute();
				$rowcount123 = $result->rowcount();

			?>
			<div style="text-align:right;">
			<h4>Total Products:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font>
			<br>
			Critical Stocks: <font style="color:red; font:bold 22px 'Aleo';">[<?php echo $rowcount123;?>]</font>	
			</div>
...
<section class="content">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("SELECT *, p_current_price * p_qty as total, p_qty < reorder_level as stock FROM tbl_product ORDER BY p_id DESC");
				$result->execute();
			...
			?></td>
			<td>₱ <?php
			$s1_price = $row['p_current_price'];
			echo formatMoney($s1_price,true);
			?></td>
			<td><?php	
			if ($row['stock']) {
				echo '<p class="alert alert-warning record" style="color: #fff; background-color:red;">';	
				echo $row['p_qty'].'  <i class="icon-arrow-down"></i>';			
			}
				else {
					echo '<p class="alert alert-warning record" style="color: #fff; background-color:green;">';	
