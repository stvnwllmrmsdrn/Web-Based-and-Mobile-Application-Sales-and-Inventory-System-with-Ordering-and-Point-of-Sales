...
$html = file_get_contents("PrintLowestEarningsWalkin.html");

// Replace the placeholder "{{data}}" with the actual data
$html = str_replace(["{{dataLowEarningsWalkin}}","{{date}}"], [$dataLowEarningsWalkin,$date], $html);
...
$dompdf->stream("Standly - Lowest Sales Product Walkin -$date .pdff", array("Attachment" => false));

// Clear any output buffers
ob_end_clean();

// Output the generated PDF to the browser
// $dompdf->stream("Standly - Lowest Sales Product Walkin -$date .pdf", ["Attachment" => 0]);
?>
