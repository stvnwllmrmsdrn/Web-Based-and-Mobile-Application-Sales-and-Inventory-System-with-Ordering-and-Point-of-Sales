...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['city_id'])) {
    	...
    	// current city name that is in the database
    	$statement = $pdo->prepare("SELECT * FROM tbl_shipping_cost WHERE shipping_cost_id=?");
		$statement->execute(array($_REQUEST['id']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach($result as $row) {
			$current_city = $row['city_id'];
		}

		$statement = $pdo->prepare("SELECT * FROM tbl_shipping_cost WHERE city_id=? and city_id!=?");
    	$statement->execute(array($_POST['city_id'],$current_city));
    	$total = $statement->rowCount();							
    	if($total) {
    		$valid = 0;
        	$error_message .= 'city already exists<br>';
    	}
    }

    if($valid == 1) {    	
		// updating into the database
		$statement = $pdo->prepare("UPDATE tbl_shipping_cost SET city_id=?,amount=? WHERE shipping_cost_id=?");
		$statement->execute(array($_POST['city_id'],$_POST['amount'],$_REQUEST['id']));
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="text"  onkeypress="return event.charCode >= 48 && event.charCode <= 57"
                                    title="Numbers only" class="form-control" name="amount" value="<?php echo $amount; ?>">
                        </div>
                    </div>
                    <div class="form-group">
